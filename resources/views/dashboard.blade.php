        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="relative bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 shadow-xl shadow-blue-500/10 overflow-hidden text-white">
                ...
                <div class="relative z-10 space-y-2">
                    ...
                    <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Welcome Back, Workspace Admin!</h1>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
                    <div class="p-3 bg-blue-50 rounded-xl text-blue-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 17.25v1.007a3 3 0 0 1-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0 1 15 18.257V17.25m6-12V15a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 15V5.25m18 0A2.25 2.25 0 0 0 18.75 3H5.25A2.25 2.25 0 0 0 3 5.25m18 0V12a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 12V5.25" /></svg></div>
                    <div><p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Labs</p><p class="text-lg font-bold text-slate-800">{{ $laboratories->count() }} Rooms</p></div>
                </div>
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
                    <div class="p-3 bg-amber-50 rounded-xl text-amber-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg></div>
                    <div><p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Occupied</p><p class="text-lg font-bold text-slate-800">{{ $laboratories->where('status', 'Occupied')->count() }} Rooms</p></div>
                </div>
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
                    <div class="p-3 bg-emerald-50 rounded-xl text-emerald-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5V6.75a4.5 4.5 0 1 1 9 0v3.75M3.75 21.75h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H3.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg></div>
                    <div><p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Available</p><p class="text-lg font-bold text-slate-800">{{ $laboratories->where('status', 'Available')->count() }} Rooms</p></div>
                </div>
            </div>
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row gap-3 items-center justify-between">
                ...
                <form method="GET" action="{{ route('dashboard') }}" class="w-full sm:w-auto flex items-center gap-2">
                    ...
                    @if(request('search'))
                        <a href="{{ route('dashboard') }}" class="bg-slate-100 text-slate-600 hover:bg-slate-200 text-xs font-bold px-4 py-2 rounded-xl transition duration-150 flex items-center justify-center">
                            Clear
                        </a>
                    @endif
                </form>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                @empty
                    <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-16 bg-white rounded-2xl border border-dashed border-slate-200 shadow-sm">
                        ...
                        <p class="text-xs text-slate-400 mt-1 max-w-xs mx-auto">We couldn't find any rooms or sections matching your search phrase: "{{ request('search') }}".</p>
                    </div>
                @endforelse
